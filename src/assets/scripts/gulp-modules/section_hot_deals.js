...
document.addEventListener('DOMContentLoaded', async function() {
  // ===== HOT DEALS DATA =====
  const wrapper = document.querySelector('#section_hot_deals .swiper-wrapper');

  if (wrapper && typeof getData === 'function') {
    const cached = sessionStorage.getItem('units');
    let allPremises = cached ? JSON.parse(cached) : await getData();

    if (!cached) {
      sessionStorage.setItem('units', JSON.stringify(allPremises));
    }
    ...
    const takeRandom = (arr, count) => shuffle(arr).slice(0, count);

    const formatNumber = val => Number(val).toLocaleString('uk-UA', { maximumFractionDigits: 0 });
    ...
    const buildUnitUrl = unit =>
      ` /flats/?project_id=${projectsIds[unit.project_name] || '0'}&id=${unit.id}`;
    ...
    const renderSlide = unit => `
      <div class="swiper-slide">
        <a href="${buildUnitUrl(unit)}"
        class="flat_card"
        data-id="${unit.id}"
      >
        ...
        <div class="flat_card__img">
          <img src="${getImageUrl(unit)}" alt="planning" />
        </div>
    ...
    const flats = allPremises.filter(u => u.unit_type_name === 'квартира');
    const apartments = allPremises.filter(u => u.unit_type_name === 'апартамент');

    const mixed = shuffle([...takeRandom(flats, 5), ...takeRandom(apartments, 5)]);
...
`;
    ...
    wrapper.innerHTML = mixed.map(renderSlide).join('') + lastSlide;
  }
