...
export function setUnits(units, wrapper, pagination, filterCards, countVisibleCards) {
  ...
  units.forEach(unit => {
    if (unit.project.name != 'Залишки') {
      const link =
        unit?.project?.id && projectIds?.[unit.project.id]
          ? `/flats?id=${unit.id}&project=${projectIds[unit.project.id]}`
          : `/flats?id=${unit.id}`;

      const unitHTML = `
        <a href=${link}
          class="flat_card" data-filtered="true"
          data-project="${unit.project?.name}" 
          data-room_count="${unit.room_count}" 
          data-type="${unit.unit_type?.name}" 
          data-total_price="${unit.total_price}"
          data-size="${unit.real_size}" 
          data-floor="${unit.floor?.order_number}" 
          data-id=${unit.id}
        >
